...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/heartbeat":
            # 轻量健康检查：返回分析文件 mtime，页面据此判断服务是否存活/是否同源数据
            try:
                mtime = int(os.path.getmtime(sys.argv[1]))
            except OSError:
                mtime = 0
            self._send(200, json.dumps({"ok": True, "mtime": mtime}), "application/json")
    ...
    def do_POST(self):
        ...
        for p in (req.get("paths") or []):
            ...
            if mode == "open" and sys.platform.startswith("win"):
                # Windows 红灯 app_paths 指向 Program Files（应用本体不在 HOME），
                # open 为非破坏性操作，放行两个 Program Files；rm/trash 白名单不受影响
                roots = roots + (os.environ.get("ProgramFiles", r"C:\Program Files"),
...
def main():
    ...
    # 单实例：同一份分析 JSON 已有存活实例时直接复用，避免多标签页指向死端口
    url_file = os.path.splitext(os.path.abspath(sys.argv[1]))[0] + ".server-url.txt"
    if os.path.exists(url_file):
        try:
            with open(url_file, encoding="utf-8") as f:
                prev = f.read().strip()
            import urllib.request
            with urllib.request.urlopen(prev + "heartbeat", timeout=2) as resp:
                info = json.load(resp)
            if info.get("ok") and info.get("mtime") == int(os.path.getmtime(sys.argv[1])):
                print("已有实例在服务同一份分析：" + prev + "（复用，不再新起）", flush=True)
                webbrowser.open(prev)
                return
        except Exception:
            pass  # 旧地址已死或文件过期 -> 正常起新实例
    ...
    # Deterministic URL handoff for detached launches (agent contexts): the
    # tokenized URL lands next to the analysis JSON regardless of stdout buffering.
    try:
        with open(os.path.splitext(os.path.abspath(sys.argv[1]))[0] + ".server-url.txt",
                  "w", encoding="utf-8") as f:
            f.write(url + "\n")
    except OSError:
        pass
    # 线程内未捕获异常落盘（ThreadingHTTPServer 默认只打到 stderr，agent 后台拉起时看不到）
    import threading
    crash_log = os.path.splitext(os.path.abspath(sys.argv[1]))[0] + ".server-crash.log"
    def _thread_hook(a):
        try:
            with open(crash_log, "a", encoding="utf-8") as f:
                f.write(time.strftime("[%Y-%m-%d %H:%M:%S] ") + repr(a.exc_type) + ": " + str(a.exc_value) + "\n")
        except Exception:
            pass
